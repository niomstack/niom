...
>(({ className, ...props }, ref) => (
    <div className="flex items-center border-b border-border-subtle px-3" cmdk-input-wrapper="">
        ...
        <CommandPrimitive.Input
            ref={ref}
            className={cn(
                "flex h-10 w-full rounded-md bg-transparent py-3 text-sm text-text-primary outline-none placeholder:text-text-muted disabled:cursor-not-allowed disabled:opacity-50",
                className
            )}
            {...props}
        />
...
>(({ className, ...props }, ref) => (
    <CommandPrimitive.Item
        ...
        className={cn(
            ...
            "data-[selected=true]:bg-surface-card-active data-[selected=true]:text-text-primary",
            "text-text-secondary",
            className
        )}
        {...props}
